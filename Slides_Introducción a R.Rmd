...
```{r}
 x <- 1
 y <- 2   
 respuesta <- (x > y)      # ¿x es mayor que y?; guardar el reultado en el objeto r
...
barplot(PlantGrowth$weight, col = "blue")
```
...
barplot(peso.mean,beside = TRUE, main = "Promedio de Crecimiento", ylab = "Peso(gr.)", xlab = "Tratamientos", col = c("royalblue", "seagreen", "purple" ))
```
...
#setwd("/home/usuario/Escritorio/Primeros-Pasos-en-R/DataBases/") #establecer la ruta al directorio en que se encuntrar los archivos que deseas leer

```
<span style="color: #00868B;">**Manos a la obra: Obtén tu directorio de trabajo actual y cambialo al diretorio DataBases **</span> 
---
...
ind_1979.sub <- subset(polinizadores, Year==1979) 
head(ind_1979)

ind_1979.conteo <- length(levels(as.factor(ind_1979$Specimen_id)))
length(ind_1979$Specimen_id)
ind_1989.sub <- subset(polinizadores, Year==1989)
ind_1989.conteo <- length(levels(as.factor(ind_1989.sub$Specimen_id)))

results.polinizadores <-as.table(cbind(ind_1979.conteo,ind_1989.conteo))

colnames(results.polinizadores) <- c("Año 1979", "Año 1989")
rownames(results.polinizadores) <- "Conteo"
results.polinizadores
